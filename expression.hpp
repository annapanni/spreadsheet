...
public:
	...
	CellRefExpr(std::string col, int row, Expression** e) : col(col), row(row), content(e) {}
	Expression** getContent() const {return content;}
	double eval() const;
	std::string show() const {return col + std::to_string(row);}
	Expression* copy() const {return new CellRefExpr(col, row, content);}
};
...
public:
	...
	Expression* copy() const {return new AvgFunc(begin, end);}
};
...
public:
	...
	Expression* copy() const {return new SumFunc(begin, end);}
};
...
public:
	...
	Expression* copy() const {return new Mult(lhs, rhs);}
};
...
public:
	...
	double eval() const {return lhs->eval() / rhs->eval();}
	std::string show() const {return "(" + lhs->show() + "/" + rhs->show() + ")";}
	Expression* copy() const {return new Div(lhs, rhs);}
};
...
public:
	...
	double eval() const {return lhs->eval() + rhs->eval();}
	std::string show() const {return "(" + lhs->show() + "+" + rhs->show() + ")";}
	Expression* copy() const {return new Add(lhs, rhs);}
};
...
public:
	...
	double eval() const {return lhs->eval() - rhs->eval();}
	std::string show() const {return "(" + lhs->show() + "-" + rhs->show() + ")";}
	Expression* copy() const {return new Sub(lhs, rhs);}
};
